...
var s_account="rdcomdev"
// if on prod env. use assigned omnitureAccount
if (typeof omnitureAccount != "null" && typeof omnitureAccount != "undefined" && omnitureAccount.valueOf() != "") {
...
s.linkInternalFilters="javascript:,www.rdasia.com,readersdigest.com.sg"
...
s.prop26=document.URL;

/* Referrer Tracking */
if (document.referrer != '' && document.referrer != 'undefined' && !(document.referrer.indexOf(document.domain) > -1)) 
{ 	
	s.prop38=document.referrer;	
}

/* Track Share-This Social Media Clicks */
window.socialCallBackFunc =function(event,service)
{
	//alert(service); //the service shared by user. e.g. facebook
		
	var s=s_gi(s_account); s.linkTrackVars='events,eVar1,eVar52'; s.linkTrackEvents='event54';s.eVar1=s.pageName;s.eVar52=service;s.events='event54';s.tl(this,'o','share-this-click>' + service);
}
if(window.stLight){
	stLight.subscribe("click",socialCallBackFunc); //register the callback function with sharethis NEED TO Check to see if stLight is defined
}
...
window.videoResume = function(){
	...
	if(window.yt_current_vid.pauseTime != -1 && Math.abs(window.yt_current_vid.pauseTime - videoCurrentTime) >1){
		...
		s.Media.play(videoTitle,parseInt(videoCurrentTime));
	}
	else{
		...
		switch(event.data)
		{
			...
			case 2:
				if(vid.videoDuration != videoCurrentTime)
				{
					...
					s.Media.stop(videoTitle,videoCurrentTime);
				}	
				break;
			case 0:
				//CLOSE
				_gaq.push(['_trackEvent', 'Videos', 'Complete', videoTitle,parseInt(vid.videoDuration)]);
				s.Media.stop(videoTitle,vid.videoDuration);
...
function s_doPlugins(s) {
	...
	s.detectRIA('s_ria','prop36','prop37');
	
	/* Add calls to plugins here */
	/* Copying Values Over - Why the values are copied over here in plugins section*/
	s.eVar24=s.prop6; //content type
	s.eVar25=s.prop30;
	s.eVar23=s.prop4;
	s.eVar19="D=s_vi";
	...
	/* Visit original trkid */
        
		if ((typeof s.eVar18 == "undefined") || (s.eVar18 == "")){
            if(s.getQueryParam('trkid')== "") {
                s.eVar18="No_original_tracking_id";
            }
            else {
                s.eVar18 = s.getQueryParam('trkid');
            }
        }
		if(!s.prop11)
			s.prop11 = s.getQueryParam('trkid');
		if(!s.campaign)
			s.campaign = s.getQueryParam('trkid,ym_mid'); 
		
		if(s.getQueryParam('wtID')!=null) {
			s.eVar14 = s.getQueryParam('wtID');
			s.prop25=s.eVar14;
		}

	/* Internal tracking id lid */

	s.prop12=s.eVar2 = s.getQueryParam('lid');
	
	/* Previous Page Name */
	var s_prevPage=s.getPreviousValue(s.pageName,"s_pv");
	s.prop41=s_prevPage?s_prevPage:'[No Previous Page Available]';
	/*prop41 is not defined in admin console*/	
	
	/* Get Visit Number */
    	s.prop39=s.eVar39=s.getVisitNum();
		
	/* New vs Repeat User + Pathing */
	s.prop33=s.getNewRepeat();
	if (s.prop33) {
		s.prop33=s.prop33+":"+s.pageName;
		s.eVar33=s.prop33;
	}
	
	/* Plugin Get Days Since Last Visit */
    	s.eVar40=s.prop40=s.getDaysSinceLastVisit();	

	/*Cross Visit participation + campaign pathing*/
	if(s.campaign)	{
		s.prop35=s.campaign+":"+s.pageName;
		s.eVar31=s.crossVisitParticipation(s.campaign,'s_ev31','30','6','>','purchase',1); 	//30 day cookie
	}
	else {
	   s.prop35="D=pageName";
	}
	...
	/* Time Parting */
	/***Time offset also varies by country so we need to get the time offset of the local country here dynamicaly***/
	...
	else if(window.location.href.indexOf(".za") != -1)
	{
	utc_offset = '+2';
	}
	...
	else if(window.location.href.indexOf(".selectionclic") != -1)
	{
	utc_offset = '+1';
	}
	...
	else if(window.location.href.indexOf(".tw") != -1)
	{
	utc_offset = '+8';
	}
	...
	else if(window.location.href.indexOf(".at") != -1)
	{
	utc_offset = '+1';
	}
	else if(window.location.href.indexOf(".ph") != -1)
	{
	utc_offset = '+8';
	}
	...
	else if(window.location.href.indexOf(".rdasia") != -1)
	{
	utc_offset = '+8';
	}
	...
	var timeZoneVar=utc_offset;

	if(timeZoneVar != '' && timeZoneVar != 'undefined'){
		temphr = s.getTimeParting('h',timeZoneVar,curdate.getFullYear());  
		tempday = s.getTimeParting('d',timeZoneVar,curdate.getFullYear()); 
	} else {
		temphr = s.getTimeParting('h','-5',curdate.getFullYear());  
		tempday = s.getTimeParting('d','-5',curdate.getFullYear()); 
	}
	if(temphr) {
	   s.prop32=tempday+":"+temphr;
	   s.eVar32="D=c32"; 
	}
...
s.trackingServer="omtrkpix.rd.com"
s.trackingServerSecure="somtrkpix.rd.com"
s.dc=122
...
s.Media.trackVars="prop6,prop50,eVar24,eVar50,eVar51,events";
s.Media.trackEvents="event60,event61,event62,event63,event64,event65,event66";
s.Media.trackMilestones="25,50,75";
s.Media.trackUsingContextData = true;
s.Media.segmentByMilestones=true;
s.Media.contextDataMapping = {
  "a.media.name":"eVar50,prop50",
  "a.media.segment":"eVar51",
  "a.contentType":"prop6,eVar24",
  "a.media.timePlayed":"event66",
  "a.media.view":"event60",
